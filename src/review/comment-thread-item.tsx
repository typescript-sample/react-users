...
import { Comment, CommentService } from "./client/comment/comment";
...
export const CommentThreadItem = ({ disable = false, resource, id, data, commentReactionService, commentThreadReactionService, commentThreadReplyService, updateCommentThread, removeCommentThread, userId }: Props) => {
  ...
  const createComment = async (e: OnClick, input: string, parent?: string) => {
    ...
    return commentThreadReplyService.comment(id, userId, data.commentId, input).then(res => {
  ...
  const updateComment = async (e: OnClick, input: string, comment: Comment) => {
    ...
    const res = await commentThreadReplyService.update(comment.commentId, input);
    if (res <= 0) {
      return storage.alert("Error")
    }
